...
def find_guides(genome: str, genome_name: str = 'N_A', PAM: str = 'NGG') -> pd.DataFrame:
    ...
    for sequence, strand in [(genome, '+'), (rev_comp, '-')]:
        sites = re.finditer('([ATGC]{20})' + PAM_regex,
                    sequence, overlapped = True, flags = re.IGNORECASE)
...
def extract_pairs(pair_coords, sites):
    ...
    sites_i = sites.loc[list(i)].rename( columns=
        dict(zip(cols, [c+'_1' for c in cols])))
    
    sites_j = sites.loc[list(j)].rename( columns=
        dict(zip(cols, [c+'_2' for c in cols])))
    
    pairs = pd.concat([
        sites_i.reset_index(drop=True),
        sites_j.reset_index(drop=True)], axis=1)
...
def find_hamming(search_sites: pd.DataFrame, guides: pd.DataFrame,
    hamming_max: int, seed_max: int, seed_size: int = 8) -> pd.DataFrame:
    ...
    if not 'One_Hot' in guides:
        guides.loc[:, 'One_Hot'] = guides['Guide'].apply(
    ...
    for i in guides.index:
        ...
        score.loc[:, 'full_mism'] = (score['One_Hot'] - target).apply(np.count_nonzero)/2
        score.loc[:, 'full_mism'] = score['full_mism'].apply(np.sum)
        score.loc[:, 'seed_mism'] = (score['Seed_One_Hot'] - target[-one_hot_seed_size:]).apply(np.count_nonzero)/2
...
def gene_pairs(gene_sites, exact_dist=62, min_dist=23, max_dist=200):
    ...
    for k, sites in gene_sites.items():
        ...
        target_pairs.loc[:, 'Distance'] = abs(target_pairs['End_1'] - target_pairs['End_2'])
        ...
        if target_pairs.empty == False:
            ...
            num_nearby = np.sum((target_pairs['Distance'] >= min_dist) &
...
if __name__ == "__main__":
    GENOME = sys.argv[1]
    TARGET = sys.argv[2]
    results = find_and_score_guides(GENOME, TARGET)
    results.to_csv()